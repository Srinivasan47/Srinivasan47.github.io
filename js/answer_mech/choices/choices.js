function choices() {
    ...
    /**
     * Choices play Model
     * Class that defines the choices play model properties
     **/
    var Choice = Backbone.Model.extend({
        ...
        renderHTMLOptions: function (data) {
            ...
            for (var i = 0; i < data.choices_available_answers.answer.length; i++) {
                var newOpt = data.choices_available_answers.answer[i].payload;
                responseElements = $(newOpt);
                if(!_.isEmpty(data.choices_available_answers.answer[i].math)){
              	  var textHint = '';
          	          ...
          	          if (imgChildren.length) {
          	            ...
          	            imgChildren.each(function(index){
          	              if ($(this).attr('math_id')) {
          	                ...
          	                for(var text_mathmlCounter = 0; text_mathmlCounter < data.choices_available_answers.answer[i].math.length; text_mathmlCounter++){
          	            	  if(data.choices_available_answers.answer[i].math[text_mathmlCounter].id == mathmlCounter){
          	            		  var mathMlVal = data.choices_available_answers.answer[i].math[text_mathmlCounter].mathml.replace(/<br>/gm, '');
          	                ...
          	                var theMathMl = '<span class="MathJax" id="MathJax-Element-answer-options-span' + mathmlCounter + '-Frame" style=""><\/span>'+'<scr' + 'ipt type="math/mml" id="MathJax-Element-answer-options-script' + mathmlCounter + '">' + mathMlVal + '<\/scr' + 'ipt>';
